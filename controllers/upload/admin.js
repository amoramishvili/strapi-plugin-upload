...
module.exports = {
  async find(ctx) {
    ...
    const files = await strapi.plugins.upload.services.upload[method](
      query,
      []
    );
  ...
  async replaceFile(ctx) {
    ...
    const data = await validateUploadBody(body);
    const replacedFiles = await uploadService.replace(
      id,
      { data, file: files },
      { user }
    );

    ctx.body = pm.sanitize(replacedFiles, {
      action: ACTIONS.read,
      withPrivate: false,
    });
  },
  ...
  async uploadFiles(ctx) {
    ...
    ctx.body = pm.sanitize(uploadedFiles, {
      action: ACTIONS.read,
      withPrivate: false,
    });
  },
